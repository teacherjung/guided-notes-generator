...
- **應試模型不該知道自己在被考**。組裝時只取準則的 A–D 四節（「出處」點名真筆記是反向工程來源、
  「待辦」直說這是一場對答案的測驗），並剝掉模板的 HTML 註解、掃過禁詞才輸出。
  兩個都是實測踩到的：註解第一行原本就寫著「盲測提示詞模板」。
...
2. 開一個**全新的** Grok 對話，上傳 `run/stewart-15.4-body.pdf`，貼上 `run/prompt-filled.md`。
3. Grok 的輸出整份存成 `run/grok-output.md`，先過機械檢查：

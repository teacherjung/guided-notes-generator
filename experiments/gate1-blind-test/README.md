...
| 角色 | 可以看 | **不可以看** |
|---|---|---|
| 應試者甲／乙（兩個不同執行模型） | 挖空準則正本、Stewart 15.4 課本頁 | CH15 (T).pdf、Calc_07 (S).pdf 的**任何**內容 |
| Claude（出題與評分表設計） | 準則、課本 | 同上 |
| William（閱卷） | 全部 | — |

- 盲測**必須開新對話**跑。同一個對話裡先聊過 15.4 的真筆記，這場就作廢。
- 提示詞裡禁止執行者上網找任何筆記風格樣本——它要從準則推，不是從樣本仿（品牌名也不進考卷，Grok 掃 #9）。
- **應試模型不該知道有參考組存在、也不該知道會被對照評分**。（Part E 要它列「準則不夠用的地方」是**刻意**告知準則可能有洞——那是 A 類情報的來源，代價是它知道準則在被檢驗；這一點是設計，不是洩題。）組裝時只取準則的 A–D 四節——其餘各節都會洩題：
...
定位課本 15.4 頁碼時只讀 Stewart 課本 PDF。（這是聲明，不是可稽核的紀錄——repo 內沒有工具呼叫日誌；守它的是規矩，不是鎖。）
...
版權紅線（PLAN.md「五、風險與紅線」第 1 條）：這些切片只給 William 個人評測用，不散布、不進 repo。
...
```
   ...
   沒過就把報告貼回去要該執行者重做——格式沒過就不該佔用 William 的紅筆時間。

4. William 紅筆：`cp experiments/gate1-blind-test/grading-sheet.md experiments/gate1-blind-test/run/grading-filled.md`，
   **先填第 0 節的 0-A／0-B（含「甲乙各是哪個模型」那格）／0-E、再看輸出**（看完才訂門檻＝沒有門檻；執行紀錄在附錄、事後填）——第 0 節裡的 **0-E 應挖點總清單**
...
| `check-output.py` | 機械檢查執行者的輸出（同構、編號、比例、三色標記）。已用壞掉的樣本實測會叫 |
